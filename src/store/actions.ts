import { Phrase, SortBy, SortOrder, ViewMode, Action } from '@types';
...
export const actions = {
  ...
  deletePhrase: (id: string): Action => ({ 
    type: 'DELETE_PHRASE', 
    payload: id 
  }),
  
  batchDelete: (ids: string[]): Action => ({ 
    type: 'BATCH_DELETE', 
    payload: ids 
  }),
  
  updatePhrase: (id: string, updates: Partial<Phrase>): Action => ({ 
    type: 'UPDATE_PHRASE', 
    payload: { id, updates } 
  }),
  ...
  togglePhraseSelection: (id: string): Action => ({ 
